...
const TaskItem = ({ item }: { item: Task }) => (
  <div className="flex items-center justify-between p-3 bg-white border border-gray-200 hover:bg-gray-50 transition-colors group cursor-move">
    <div className="flex items-center space-x-3">
      ...
      <span className="text-gray-500 font-medium text-xs w-16 truncate" title={item.id}>{item.id.substring(0, 8)}</span>
    ...
    <div className="flex items-center space-x-4">
      {/* Status Badge */}
      <span className={`px-2 py-0.5 text-xs font-bold uppercase rounded ${item.status === 'To Do' ? 'bg-gray-200 text-gray-700' :
...
export default function BacklogView({ tasks, onTaskCreate }: BacklogViewProps) {
      ...
      <div className="bg-gray-50 rounded-lg p-2">
        ...
        {isSprintOpen && (
          <div className="space-y-[-1px]"> {/* Negative margin to merge borders */}
            {sprintTasks.map(item => <TaskItem key={item.id} item={item} />)}
            <button onClick={onTaskCreate} className="w-full text-left p-2 pl-9 text-sm text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-colors">
              + Create issue
            </button>
          </div>
        )}
      ...
      <div className="bg-gray-50 rounded-lg p-2">
        ...
        {isBacklogOpen && (
          <div className="space-y-[-1px]">
            {backlogTasks.map(item => <TaskItem key={item.id} item={item} />)}
            <button onClick={onTaskCreate} className="w-full text-left p-2 pl-9 text-sm text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-colors">
              + Create issue
            </button>
          </div>
